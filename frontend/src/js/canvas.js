...
(function () {
    ...
    function init() {
        ...
        /* ResizeObserver garante que só desenha quando o container
           realmente tem tamanho, resolve o problema de timing entre abas, todavia podia ser uma solução melhjor*/
    ...
    function calcularTransformacao(nos) {
        ...
        escala = Math.min(
            (largura  - 2 * MARGEM) / spanX,
            (altura   - 2 * MARGEM) / spanY,
            150
        );
        ...
        offsetY = altura  / 2 + cy * escala;
    }

    function tx(x) { return x *  escala + offsetX; }
    function ty(y) { return y * -escala + offsetY; }

    
    function desenhar() {
        if (!ctx || !iniciado) return;
    ...
    function desenharBarras(nos, barras) {
        ...
        barras.forEach(b => {
            const nA = mapa[b.noA], nB = mapa[b.noB];
            if (!nA || !nB) return;
    ...
    function desenharApoios(nos, apoios) {
        ...
        apoios.forEach(apoio => {
            const no = mapa[apoio.no ?? apoio.noId];
            if (!no) return;
            ...
            ctx.fillStyle   = COR_APOIO;
            ctx.strokeStyle = COR_APOIO;
            ctx.lineWidth   = 2;
    ...
    function desenharCargas(nos, cargas) {
        ...
        cargas.forEach(carga => {
            const no = mapa[carga.no ?? carga.noId];
            if (!no) return;

            const xNo = tx(no.x), yNo = ty(no.y);
            ...
            ctx.lineTo(
                xNo - Math.cos(angRad) * RAIO_NO,
                yNo + Math.sin(angRad) * RAIO_NO
            );
            ctx.stroke();

           
            const angTela = Math.atan2(
                yNo - yOrig,
                xNo - xOrig
            );
            const xPonta = xNo - Math.cos(angRad) * RAIO_NO;
            const yPonta = yNo + Math.sin(angRad) * RAIO_NO;

            ctx.beginPath();
            ctx.moveTo(xPonta, yPonta);
            ctx.lineTo(xPonta - Math.cos(angTela - 0.4) * PONTA,
                       yPonta - Math.sin(angTela - 0.4) * PONTA);
            ctx.lineTo(xPonta - Math.cos(angTela + 0.4) * PONTA,
                       yPonta - Math.sin(angTela + 0.4) * PONTA);
            ctx.closePath();
            ctx.fill();
            ...
            ctx.textBaseline = 'middle';
            ctx.fillText(
                `${carga.rotulo} (${carga.magnitude}N)`,
                xOrig - Math.cos(angRad) * 14,
                yOrig + Math.sin(angRad) * 14
            );
        });
    }
